...
var.p =
  ...
  ylab(" ")+
  #coord_cartesian(xlim=c(-0.01,0.44))+
  #scale_x_continuous(expand=c(0,0), breaks=seq(0,0.6,by=0.2))+
...
#express selection for or against variance (reciprocal) of disc in ordinal model) 
#ldf[ldf$variable=="v", "value"] = -1 * ldf[ldf$variable=="v", "value"]
#ldf[ldf$variable=="m.v", "value"] = -1 * ldf[ldf$variable=="m.v", "value"]
...
NLS_sdplot =
  ...
  #coord_cartesian(xlim=c(-0.4,0.4)) + scale_x_continuous(expand = c(0,0))+
...
#############################################################
#Plot date effects on selection
#############################################################

#load models
samplesKY = readRDS("NLS_KYd_m.RDS")
samplesLY = readRDS("NLS_LYd_m.RDS")

#date-specific selection gradients
date = c(-2, 0, 2)

#initialize lists for date
bhKY = list()
ghKY = list()
bsKY = list()
gsKY = list()
...
for(k in 1:length(date)){
  ...
  X_env = as.matrix(cbind(stan_data_KY$dateWSD, stan_data_KY$attemptSD,
                          stan_data_KY$dateWSD * stan_data_KY$attemptSD), ncol = 3)
  #environmental effects
  env_hatch = draws_of(samplesKY$coef_h)
  env_survive = draws_of(samplesKY$coef_s)
  env_mass = draws_of(samplesKY$coef_m)
  
  #individual effects (unexplained selection)
  id_hatch = draws_of(samplesKY$I_h_0)
  id_survive = draws_of(samplesKY$I_s_0)
  id_mass = draws_of(samplesKY$I_m_0)
  
  #year effects
  year_hatch = draws_of(samplesKY$Y_h_0)
  year_survive = draws_of(samplesKY$Y_s_0)
  year_mass = draws_of(samplesKY$Y_m_0)
  
  #overdispersion effects
  ovd_survive = draws_of(samplesKY$O_s_0)
    ...
    Wbar = function(z,i) {
      eta =     ( #latent mean (zero-centered, no intercept)
        X_env %*% env_hatch[i,] + #environmental effects
          ...
          z[1] * (draws_of(samplesKY$b_h)[i,1] + draws_of(samplesKY$db_h)[i,1] * date[[k]] ) + #linear selection coef
          z[2] * (draws_of(samplesKY$b_h)[i,2] + draws_of(samplesKY$db_h)[i,2] * date[[k]] ) +
          
          z[1]^2 * (draws_of(samplesKY$q_h)[i,1] + draws_of(samplesKY$dq_h)[i,1] * date[[k]]) + #nonlin selection coef
          z[2]^2 * (draws_of(samplesKY$q_h)[i,2] + draws_of(samplesKY$dq_h)[i,2] * date[[k]] ) +
          z[1] * z[2] * (draws_of(samplesKY$q_h)[i,3] + draws_of(samplesKY$dq_h)[i,3] * date[[k]] )
      )  
      
      #calculate probabilities for each hatchling count
      nthres = stan_data_KY$nthres_hatch
      d = exp(draws_of(samplesKY$int_h)[i])
      p = matrix(NA, nrow = length(eta), ncol = nthres+1)
      for(j in 1:nthres){
        if(j == 1){p[,j] = logistic( (draws_of(samplesKY$mu_h_0)[i,j] - eta) / d ) }
        else p[,j] = logistic( (draws_of(samplesKY$mu_h_0)[i,j] - eta) / d ) - 
            logistic(  (draws_of(samplesKY$mu_h_0)[i,j-1] - eta) / d )
      }
      p[,nthres+1] = 1 - rowSums(p[,1:nthres])
  ...
  for(i in 1:nrow(env_survive) ){
    Wbar = function(z,i) {
      p = logistic(
        draws_of(samplesKY$mu_s_0)[i] +
          X_env %*% env_survive[i,] + #environmental effects
          
          id_survive[i,stan_data_KY$id] + #unexplained selection
          year_survive[i,stan_data_KY$year_id] + #yearly effects  
          
          z[1] * (draws_of(samplesKY$b_s)[i,1] + draws_of(samplesKY$db_s)[i,1] * date[[k]] ) + #linear selection coef
          z[2] * (draws_of(samplesKY$b_s)[i,2] + draws_of(samplesKY$db_s)[i,2] * date[[k]] ) +
          
          z[1]^2 * (draws_of(samplesKY$q_s)[i,1] + draws_of(samplesKY$dq_s)[i,1] * date[[k]]) + #nonlin selection coef
          z[2]^2 * (draws_of(samplesKY$q_s)[i,2] + draws_of(samplesKY$dq_s)[i,2] * date[[k]] ) +
          z[1] * z[2] * (draws_of(samplesKY$q_s)[i,3] + draws_of(samplesKY$dq_s)[i,3] * date[[k]] ) +
          ovd_survive[i,])
      return(mean(p))  }
    z = rep(0,2)
    first.derivatives = grad(func = Wbar, x = z, i = i)
  ...
  for(i in 1:nrow(env_survive) ){
    Wbar = function(z,i) {
      p = 
        draws_of(samplesKY$mu_m_0)[i] +
        X_env %*% env_mass[i,] + #environmental effects
        
        id_mass[i,stan_data_KY$id] + #unexplained selection
        year_mass[i,stan_data_KY$year_id] + #yearly effects  
        
        z[1] * (draws_of(samplesKY$b_m)[i,1] + draws_of(samplesKY$db_m)[i,1] * date[[k]] ) + #linear selection coef
        z[2] * (draws_of(samplesKY$b_m)[i,2] + draws_of(samplesKY$db_m)[i,2] * date[[k]] ) +
        
        z[1]^2 * (draws_of(samplesKY$q_m)[i,1] + draws_of(samplesKY$dq_m)[i,1] * date[[k]]) + #nonlin selection coef
        z[2]^2 * (draws_of(samplesKY$q_m)[i,2] + draws_of(samplesKY$dq_m)[i,2] * date[[k]] ) +
        z[1] * z[2] * (draws_of(samplesKY$q_m)[i,3] + draws_of(samplesKY$dq_m)[i,3] * date[[k]] )
      return(mean(p))  }
    z = rep(0,2)
    first.derivatives = grad(func = Wbar, x = z, i = i)
    second.derivatives = hessian(func = Wbar, x = z, i = i)
  ...
  #convert selection gradients to standardized gradients
  sd_selKY = data.frame(sd_m = apply(draws_of(samplesKY$I_c)[,,1],1, FUN=sd),
                        sd_v = apply(draws_of(samplesKY$I_c)[,,2],1, FUN=sd),
                        sd_m.m = apply(draws_of(samplesKY$I_c)[,,1]^2,1, FUN=sd),
                        sd_v.v = apply(draws_of(samplesKY$I_c)[,,2]^2,1, FUN=sd),
                        sd_m.v = apply(draws_of(samplesKY$I_c)[,,1]*draws_of(samplesKY$I_c)[,,2],1, FUN=sd))
  ...
  bhKY[[k]] = betas_sd_hatchKY
  ghKY[[k]] = gammas_sd_hatchKY
  bsKY[[k]] = betas_sd_surviveKY
  gsKY[[k]] = gammas_sd_surviveKY
  bmKY[[k]] = betas_sd_massKY
  gmKY[[k]] = gammas_sd_massKY
}

saveRDS(bhKY, "bhKY.RDS")
saveRDS(ghKY, "ghKY.RDS")
saveRDS(bsKY, "bsKY.RDS")
saveRDS(gsKY, "gsKY.RDS")
saveRDS(bmKY, "bmKY.RDS")
saveRDS(gmKY, "gmKY.RDS")

dfs = list()
for(k in 1:length(date)){
  dfk =  cbind(bhKY[[k]], bsKY[[k]], bmKY[[k]],
              ghKY[[k]], ghKY[[k]], ghKY[[k]])
  
  colnames(dfk) = c("bm_h", "bv_h", 
                    "bm_s", "bv_s", 
                    "bm_m", "bv_m",
                    "qmm_h", "qvv_h", "qmv_h", 
                    "qmm_s", "qvv_s", "qmv_s", 
  ...
  dfk = reshape2::melt(dfk)
  dfk$date = date[[k]]
  dfs[[k]] = dfk
}
dfk = do.call("rbind", dfs)

library(stringr)
dfk$type = str_sub(dfk$variable, 1, 1)
dfk$par = gsub("_", "", str_sub(dfk$variable, 2, 3))
dfk$comp = gsub("_", "", str_sub(dfk$variable, 4, 4))
dfk$pop = "KY"  

#subset to relevant changes
dfks = dfk[dfk$variable %in% c("bm_h","bv_h", "bm_s", "bm_m", "qvv_m"), ]

dfks = dfk[dfk$variable %in% c("bm_h","bv_h", "qvv_m"), ]

ggplot(dfks, aes(x = value, fill = date, color = date), )+
  facet_wrap(. ~ comp * par, nrow = 3)+
  geom_density(aes(y = after_stat(scaled), group = date, alpha = 0.4))
