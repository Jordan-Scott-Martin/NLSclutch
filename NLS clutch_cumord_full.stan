...
transformed data {
  ...
  int<lower=1> ntrait = 5; //number of RN components under selection (intercepts and residuals)
...
model {
  ...
  vector[I] I_c_p1 = col(I_c, 3); //variability
  vector[I] I_c_p2 = col(I_c, 4); //variability
  vector[I] I_c_p3 = col(I_c, 5); //variability
  ...
  for (n in 1 : N) {
    mu_c[n] += Y_c_0[year_id[n]] + I_c_mu[id[n]] + 
                I_c_p1[id[n]] * col(X_clutch, 1)[n] +
                I_c_p2[id[n]] * col(X_clutch, 2)[n] +
                I_c_p3[id[n]] * col(X_clutch, 3)[n] ;
  ...
  for (n in 1 : N) {
    mu_h[n] +=   b_h[1] * I_c_mu[id[n]] + b_h[2] * I_c_disc[id[n]]
               +  b_h[3] * I_c_p1[id[n]] + b_h[4] * I_c_p2[id[n]] + b_h[5] * I_c_p3[id[n]]
               + q_h[1] * square(I_c_mu[id[n]])
               + q_h[2] * square(I_c_disc[id[n]])
               + q_h[3] * square(I_c_p1[id[n]])
               + q_h[4] * square(I_c_p2[id[n]])
               + q_h[5] * square(I_c_p3[id[n]])
               + q_h[6] * (I_c_mu[id[n]] * I_c_disc[id[n]])
               + q_h[7] * (I_c_mu[id[n]] * I_c_p1[id[n]])
               + q_h[8] * (I_c_mu[id[n]] * I_c_p2[id[n]])
               + q_h[9] * (I_c_mu[id[n]] * I_c_p3[id[n]])
               + q_h[10] * (I_c_disc[id[n]] * I_c_p1[id[n]])
               + q_h[11] * (I_c_disc[id[n]] * I_c_p2[id[n]])
               + q_h[12] * (I_c_disc[id[n]] * I_c_p3[id[n]])
               + q_h[13] * (I_c_p1[id[n]] * I_c_p2[id[n]])
               + q_h[14] * (I_c_p1[id[n]] * I_c_p3[id[n]])
               + q_h[15] * (I_c_p2[id[n]] * I_c_p3[id[n]])
               ;
  }
  
  for (n in 1 : N) {
    mu_s[n] +=   b_s[1] * I_c_mu[id[n]] + b_s[2] * I_c_disc[id[n]]
               +  b_s[3] * I_c_p1[id[n]] + b_s[4] * I_c_p2[id[n]] + b_s[5] * I_c_p3[id[n]]
               + q_s[1] * square(I_c_mu[id[n]])
               + q_s[2] * square(I_c_disc[id[n]])
               + q_s[3] * square(I_c_p1[id[n]])
               + q_s[4] * square(I_c_p2[id[n]])
               + q_s[5] * square(I_c_p3[id[n]])
               + q_s[6] * (I_c_mu[id[n]] * I_c_disc[id[n]])
               + q_s[7] * (I_c_mu[id[n]] * I_c_p1[id[n]])
               + q_s[8] * (I_c_mu[id[n]] * I_c_p2[id[n]])
               + q_s[9] * (I_c_mu[id[n]] * I_c_p3[id[n]])
               + q_s[10] * (I_c_disc[id[n]] * I_c_p1[id[n]])
               + q_s[11] * (I_c_disc[id[n]] * I_c_p2[id[n]])
               + q_s[12] * (I_c_disc[id[n]] * I_c_p3[id[n]])
               + q_s[13] * (I_c_p1[id[n]] * I_c_p2[id[n]])
               + q_s[14] * (I_c_p1[id[n]] * I_c_p3[id[n]])
               + q_s[15] * (I_c_p2[id[n]] * I_c_p3[id[n]])
               ;
  }
  
  for (n in 1 : N) {
    mu_m[n] +=   b_m[1] * I_c_mu[id[n]] + b_m[2] * I_c_disc[id[n]]
               +  b_m[3] * I_c_p1[id[n]] + b_m[4] * I_c_p2[id[n]] + b_m[5] * I_c_p3[id[n]]
               + q_m[1] * square(I_c_mu[id[n]])
               + q_m[2] * square(I_c_disc[id[n]])
               + q_m[3] * square(I_c_p1[id[n]])
               + q_m[4] * square(I_c_p2[id[n]])
               + q_m[5] * square(I_c_p3[id[n]])
               + q_m[6] * (I_c_mu[id[n]] * I_c_disc[id[n]])
               + q_m[7] * (I_c_mu[id[n]] * I_c_p1[id[n]])
               + q_m[8] * (I_c_mu[id[n]] * I_c_p2[id[n]])
               + q_m[9] * (I_c_mu[id[n]] * I_c_p3[id[n]])
               + q_m[10] * (I_c_disc[id[n]] * I_c_p1[id[n]])
               + q_m[11] * (I_c_disc[id[n]] * I_c_p2[id[n]])
               + q_m[12] * (I_c_disc[id[n]] * I_c_p3[id[n]])
               + q_m[13] * (I_c_p1[id[n]] * I_c_p2[id[n]])
               + q_m[14] * (I_c_p1[id[n]] * I_c_p3[id[n]])
               + q_m[15] * (I_c_p2[id[n]] * I_c_p3[id[n]])
               ;
  }
